...
public class MyFFmpegVideoEndPoint : IVideoSink, IDisposable
{
    public ILogger logger = Logger.Default;
    ...
    private MyFFmpegVideoEncoder _ffmpegEncoder;
    ...
    public MyFFmpegVideoEndPoint(Dictionary<string, string>? decoderOptions = null)
    {
        _videoFormatManager = new MediaFormatManager<VideoFormat>(_supportedFormats);
        _ffmpegEncoder = new MyFFmpegVideoEncoder(decoderOptions);
    }
    ...
    {
        return this.OnVideoSinkDecodedSampleFaster != null;
    }
    ...
    public void SetDecoderWrapper(string wrapperName)
    {
        ...
        logger.Error("TEST", "Video Encoder is not yet initialized.");
    ...
    public bool SetDecoderForCodec(VideoCodecsEnum codec, string name, Dictionary<string, string>? opts = null)
    {
        if (_ffmpegEncoder != null)
        {
            AVCodecID? aVCodecID = FFmpegConvert.GetAVCodecID(codec);
            if (aVCodecID.HasValue)
            {
                return _ffmpegEncoder.SetCodec(aVCodecID.Value, name, opts);
            }

            logger.Error("TEST", $"Codec {codec} is not supported by this endpoint.");
    ...
    {
        if (_isClosed || payload == null || this.OnVideoSinkDecodedSampleFaster == null)
        {
            return;
        }
        ...
        int width;
        int height;
        List<RawImage>? list = _ffmpegEncoder.DecodeFaster(aVCodecID.Value, payload, out width, out height);
        if (list == null || width == 0 || height == 0)
        {
            logger.Error("TEST", $"Decode of video sample failed, width {width}, height {height}.");
            return;
        }

        foreach (RawImage item in list)
        {
            this.OnVideoSinkDecodedSampleFaster?.Invoke(item);
        }
    }
...
public sealed unsafe class MyFFmpegVideoEncoder : IVideoEncoder, IDisposable
{
    ...
    private ILogger logger = Logger.Default;

    public MyFFmpegVideoEncoder(Dictionary<string, string>? encoderOptions = null, AVHWDeviceType HWDeviceType = AVHWDeviceType.AV_HWDEVICE_TYPE_NONE)
    {
    ...
    public bool SetCodec(AVCodecID cdc, string name, Dictionary<string, string>? opts = null)
    {
        ...
        if (codec == null)
        {
            logger.Error("TEST", $"Encoder for {cdc} with name '{name}' is Not Found.");
            return false;
        }
    ...
    private AVCodec* GetCodec(AVCodecID codecID, string? wrapName, bool isEncoder = true)
    {
        ...
        if (cdc == null)
            logger.Error("TEST", $"Codec not found for {codecID} with wrapper {_wrapName}");
    ...
    public void InitialiseEncoder(AVCodecID codecID, int width, int height, int fps)
    {
        if (!_isEncoderInitialised)
        {
            ...
            {
                logger.Error("TEST", $"Failed to set default encoder options for codec {cdcname}. {ex.Message}");
                throw;
            }
            ...
            foreach (var option in _codecOptions)
            {
                ...
                if (ok < 0)
                    logger.Error("TEST", $"Failed to set encoder option \"{option.Key}\"=\"{option.Value}\", Skipping this option. {FFmpegInit.av_strerror(ok)}");
            }

            ffmpeg.avcodec_open2(_encoderContext, codec, null).ThrowExceptionIfError();

            logger.Log("TEST", $"Successfully initialised ffmpeg based image encoder: CodecId:[{codecID}] - Name:[{GetNameString(codec->name)}] - {width}:{height} - {fps} Fps - {_encoderContext->pix_fmt}");
        }
    }
    ...
    private void InitialiseDecoder(AVCodecID codecID)
    {
        if (!_isDecoderInitialised)
        {
            ...
            foreach (var option in decOpts)
            {
                ...
                {
                    logger.Error("TEST", $"Failed to set decoder option \"{option.Key}\"=\"{option.Value}\", Skipping this option. {excp.Message}");
                }
            };

            ffmpeg.avcodec_open2(_decoderContext, codec, null).ThrowExceptionIfError();

            logger.Log("TEST", $"Successfully initialised ffmpeg based image decoder: CodecId:[{codecID}] - Name:[{GetNameString(codec->name)}]");
        }
    }
        ...
        if (!_isDisposed)
        {
            ...
            lock (_encoderLock)
            {
                ...
                try
                {
                    ...
                    {
                        logger.Log("TEST", "Video encoder needs more data.");
                        return null;
                    }
                    else
                    {
    ...
    */
    ...
    public void Dispose()
    {
        logger.Log("TEST", "VideoEncoder dispose.");
    ...
    internal bool NegotiatePixelFormat(AVCodecID codecid, int width, int height, int frameRate, AVPixelFormat[]? sourcePixFmts, out AVPixelFormat fmt)
    {
        ...
        lock (_encoderLock)
        {
            ...
            logger.Log("TEST", $"Negotiating pixel format for codec [{GetNameString(_encoderContext->codec->name)}].");
            ...
            {
                logger.Log("TEST", $"Skipping unsupported pixel format {*fmts}.");
                fmts++;
            }
            ...
            logger.Log("TEST", $"Negotiated pixel format {fmt}");
